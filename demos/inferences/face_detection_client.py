#!/usr/bin/env python3
"""
CLI client for face detection inference workflow.

Usage:
    python face_detection_client.py <image_path> --media-store <host>:<port> [options]

Example:
    python face_detection_client.py people.jpg --media-store localhost:8000
    python face_detection_client.py group_photo.jpg --media-store 192.168.1.100:8000 --timeout 600
"""
...
def main():
    """Main entry point for face detection client."""
    parser = argparse.ArgumentParser(
        description="Detect faces in images using the inference service",
...
Examples:
  ...
  python face_detection_client.py photo.jpg --media-store 192.168.1.10:8000 --inference 192.168.1.10:8001 --timeout 600

  # Add a label for the image in media_store
  python face_detection_client.py group.jpg --media-store localhost:8000 --label "family_photo"

  # High priority job
  python face_detection_client.py urgent.jpg --media-store localhost:8000 --priority 9
        """,
    )
