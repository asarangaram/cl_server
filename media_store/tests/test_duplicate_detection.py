...
class TestDuplicateDetection:
    ...
    def test_duplicate_upload_rejected(self, client, sample_image, clean_media_dir):
        ...
        assert md5_hash in error_data["detail"]
        assert str(entity_id) in error_data["detail"]
        ...
        for image_path in sample_images:
            with open(image_path, "rb") as f:
                response = client.post(
                    ...
                    data={"is_collection": "false", "label": f"Upload {image_path.name}"}
                )
    ...
    def test_put_with_duplicate_file_rejected(self, client, sample_images, clean_media_dir):
        ...
        assert md5_2 in error_data["detail"]
        assert str(entity2_id) in error_data["detail"]
    
    def test_put_same_entity_with_same_file_allowed(self, client, sample_image, clean_media_dir):
