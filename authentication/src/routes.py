...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    access_token = auth_utils.create_access_token(
        data={"sub": user.username, "permissions": permissions, "is_admin": user.is_admin},
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db), current_user = Depends(get_current_admin_user)):
    user_service = service.UserService(db)
...
def update_user(user_id: int, user_update: schemas.UserUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_admin_user)):
    user_service = service.UserService(db)
